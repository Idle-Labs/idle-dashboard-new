...
export class BestYieldVault {
  ...
  constructor(props: ConstructorProps) {
    ...
    this.rewardTokens = [];
    this.rewardTokens = tokenConfig.autoFarming
      ? tokenConfig.autoFarming.reduce(
          (rewards: UnderlyingTokenProps[], rewardToken: string) => {
    ...
    this.contract = new web3.eth.Contract(
      this.idleConfig.abi,
      this.idleConfig.address
    );
    if (web3Rpc) {
      this.contractRpc = new web3Rpc.eth.Contract(
        this.idleConfig.abi,
        this.idleConfig.address
      );
    }
      ...
      async (internalTxs: Transaction[]) => {
        ...
        for (const tx of internalTxs) {
          ...
          const isDepositTx =
            ...
            tx.to.toLowerCase() ===
              this
                .id; /* || (depositProxyContractInfo && tx.to.toLowerCase() === depositProxyContractInfo.address.toLowerCase() && internalTxs.filter(iTx => iTx.contractAddress.toLowerCase() === this.id).length > 0)*/
          const isRedeemTx =
            isRightToken &&
            !isDepositInternalTx &&
          ...
          const isWithdrawTx =
            internalTxs.length > 1 &&
            internalTxs.filter((iTx) =>
              this.tokenConfig.protocols
                .map((p) => p.address.toLowerCase())
                .includes(iTx.contractAddress.toLowerCase())
            ).length > 0 &&
            tx.contractAddress.toLowerCase() === this.id;
          ...
          const isSwapOutTx =
            !isSendTransferTx &&
            !isWithdrawTx &&
            !isRedeemInternalTx /*&& !etherscanTxs[tx.hash]*/ &&
            tx.from.toLowerCase() === account.toLowerCase() &&
            tx.contractAddress.toLowerCase() === this.id;
          const isSwapTx =
            !isReceiveTransferTx &&
            !isDepositInternalTx /*&& !etherscanTxs[tx.hash]*/ &&
            tx.to.toLowerCase() === account.toLowerCase() &&
            tx.contractAddress.toLowerCase() === this.id;
          ...
          const subAction = Object.keys(subActions).find(
          ...
          if (action) {
            ...
            const underlyingTokenTx = internalTxs.find((iTx) => {
              ...
              ); /*|| this.tokenConfig.proxies?.includes(underlyingTokenDirectionAddress.toLowerCase()*/
            });
              ...
              if (getTokenPrice) {
                ...
                try {
                  ...
                  const callback = async () =>
                    await catchPromise(
                      // @ts-ignore
                      pricesCalls[0].call.call({}, +tx.blockNumber)
                    );
  ...
  public getInterestBearingTokensExchangeRatesCalls(): ContractRawCall[] {
    return this.tokenConfig.protocols.reduce(
      (calls: ContractRawCall[], protocolToken: IdleTokenProtocol) => {
        // Exchange rate call
        const exchangeRateFunction = protocolToken.functions?.exchangeRate;
        if (exchangeRateFunction) {
          const protocolAbi = protocolToken.abi
            ? (protocolToken.abi as Abi)
            : (ERC20 as Abi);
          const protocolAddress =
            exchangeRateFunction.target || protocolToken.address;
          const protocolTokenContract = new this.web3.eth.Contract(
            protocolAbi,
            protocolAddress
          );
          if (protocolTokenContract.methods[exchangeRateFunction.name]) {
            calls.push({
              ...
              data: {
                ...
                decimals:
                  exchangeRateFunction.decimals || protocolToken.decimals,
              },
              call: protocolTokenContract.methods[exchangeRateFunction.name](
                ...exchangeRateFunction.params
              ),
            });
          }
        }
  ...
  public getInterestBearingTokensCalls(): ContractRawCall[] {
    const calls = this.tokenConfig.protocols.reduce(
      (calls: ContractRawCall[], protocolToken: IdleTokenProtocol) => {
        const protocolTokenContract = new this.web3.eth.Contract(
          ERC20 as Abi,
  ...
  public async getHistoricalData(
    ...
    return await this.vaultFunctionsHelper.getVaultHistoricalDataFromIdleApi(
      this,
      filters
    );
  }
  ...
  public async getHistoricalPrices(
    ...
    return await this.vaultFunctionsHelper.getVaultPricesFromIdleApi(
      this,
      filters
    );
  }

  public async getHistoricalAprs(
    filters?: PlatformApiFilters
  ): Promise<VaultHistoricalRates> {
    return await this.vaultFunctionsHelper.getVaultRatesFromIdleApi(
      this,
      filters
    );
  }
  ...
  public getFlag(flag: string, defaultVal?: any): any {
    return getObjectPath(this, `flags.${flag}`, defaultVal);
  }
  ...
  public getWithdrawInterestBearingContractSendMethod(
    params: any[] = []
  ): ContractSendMethod {
