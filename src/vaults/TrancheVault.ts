...
export class TrancheVault {
  ...
  constructor(props: ConstructorProps) {
    ...
    this.trancheContract = new web3.eth.Contract(
      this.trancheConfig.abi,
      this.trancheConfig.address
    );
  }
  ...
  public getDiscountedFees(
    account: string,
    etherscanTransactions: EtherscanTransaction[]
  ): EtherscanTransaction[] {
    const tokenAddr = this.id; // his.underlyingToken?.address
    if (
      !this.flags?.feeDiscountEnabled ||
      !tokenAddr ||
      !distributedFeesSenders[this.chainId]
    )
      return [];
    return etherscanTransactions.filter((tx: EtherscanTransaction) => {
      return (
        cmpAddrs(tx.contractAddress, tokenAddr as string) &&
        Object.keys(distributedFeesSenders[this.chainId])
    ...
    >(
      ...
      async (internalTxs: Transaction[]) => {
        ...
        for (const tx of internalTxs) {
          ...
          const isGaugeDeposit =
            this.gaugeConfig &&
            tx.contractAddress.toLowerCase() === this.id &&
            tx.to.toLowerCase() === this.gaugeConfig.address.toLowerCase();
          const isGaugeRedeem =
            this.gaugeConfig &&
            tx.contractAddress.toLowerCase() === this.id &&
            tx.from.toLowerCase() === this.gaugeConfig.address.toLowerCase() &&
  ...
  public getPoolCustomCalls(
    methodName: string,
    params: any[],
    data: any
  ): ContractRawCall[] {
    if (!this.poolContract || !this.poolContract?.methods?.[methodName])
      return [];
    return [
      {
        data,
  ...
  public getPoolDataCalls(): ContractRawCall[] {
    ...
    return protocolFields.map((protocolField: ProtocolField) => ({
      ...
      data: {
        ...
        decimals:
          protocolField.decimals || this.underlyingToken?.decimals || 18,
      },
  ...
  public getPoolVaultOpen(): ContractRawCall[] {
    if (!this.poolContract || !this.poolConfig?.functions?.vaultIsOpen)
      return [];
    return [
      {
        assetId: this.id,
        call: this.poolContract.methods[
          this.poolConfig.functions.vaultIsOpen
        ](),
      },
    ];
  }
  ...
  public async getHistoricalData(
    ...
    return await this.vaultFunctionsHelper.getVaultHistoricalDataFromSubgraph(
      this,
      filters
    );
  }
  ...
  public async getHistoricalPrices(
    ...
    return await this.vaultFunctionsHelper.getVaultPricesFromSubgraph(
      this,
      filters
    );
  }
  ...
  public async getHistoricalAprs(
    ...
    return await this.vaultFunctionsHelper.getVaultRatesFromSubgraph(
      this,
      filters
    );
  }
    ...
    return {
      ...
      [this.id]: {
        ...
        name:
          ...
          this.trancheConfig.label ||
          this.trancheConfig.token,
      },
    };
  }
