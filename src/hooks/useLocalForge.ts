...
type HookMethods = [any, (value: any) => void, () => void, boolean, boolean];

export default function useLocalForge<T = any>(
  key: string,
  initialValue?: T
): HookMethods {
  ...
  const [storedValue, setStoredValue] =
    useState<T | undefined | null>(initialValue);
  ...
  useEffect(() => {
    (async function () {
      try {
        const value: T | undefined | null = await localforage.getItem(key);
