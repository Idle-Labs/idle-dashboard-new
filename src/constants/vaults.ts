    ...
    */
    ...
    amphor:{
      wstETH:{
        ...
        description:'This strategy deploys funds in the <a href="https://app.amphor.io/earn" class="link" rel="nofollow noopener noreferrer" target="_blank">Amphor ETH/USDC</a> pool.<br />The synthetic LP replicates the payoff of a position in Uniswap v3 liquidity pool while hedging against Impermanent Loss on ETH/USDC.<br />As long as ETH/USDC is trading above the "Risk threshold", the vault generates <span class="bold" style="color: #fff;">{apy}% APR</span>.<br />Every week, the vault early terminates if ETH price closes higher the "Termination threshold".<br />If the vault reaches its maturity and ETH price ends below the "Risk threshold", on <span class="bold" style="color: #fff;">{epochEnd}</span>, the capital would be at risk. To prevent any loss, the position will restructure if the ETH price gets too close to the "Risk threshold".',
        Tranches:{
          AA:{
            abi:ERC20 as Abi,
            decimals:18,
